...
for _env_file in sorted(_root_dir.glob("*.env")):
    _env_names.append(_env_file.name)
...
# Phase 2: Load Rich and console UI for beautiful output
from rich.console import Console
from proxy_app.console_ui import (
    console,
...
print_startup_banner(
    ...
    github_url="https://github.com/Mirrowel/LLM-API-Key-Proxy"
)
...
print_config_summary(
    ...
    port=args.port,
    proxy_api_key=proxy_api_key,
    env_files=_env_names,
    github_url="https://github.com/Mirrowel/LLM-API-Key-Proxy"
)

console.print(f"[bold cyan]  ➜ Dashboard available at: http://{args.host}:{args.port}[/bold cyan]")

# Phase 3: Heavy dependencies with loading status
_verbose = getattr(args, 'verbose', False)

with console.status("[dim]Loading dependencies...[/]", spinner="dots") as status:
    if _verbose:
        status.update("[dim]Loading FastAPI framework...[/]")
...
<<<<<<< HEAD
    ...
    if _verbose:
        status.update("[dim]Discovering provider plugins...[/]")
...
class CleanConsoleHandler(logging.Handler):
    """Console handler that filters verbose messages and deduplicates repeated warnings."""
    ...
    SKIP_PATTERNS = [
        ...
        "Getting available models for provider",
        "Returning cached models",
        "Got ",  # "Got X models for provider"
        "Finished getting all",
        "Filtered out",
        # Provider initialization
        "Provider '",  # Provider rotation mode messages
        "Starting OAuth credential",
        "OAuth credential processing",
        "RotatingClient initialized",
        "RotatingClient and EmbeddingBatcher",
        ...
        "Registry refresh",
        # Credential acquisition/release (runtime)
        "Acquiring key",
    ...
    # Patterns that should be deduplicated with counters
    DEDUP_PATTERNS = [
        "No keys are eligible",
        "Missing thoughtSignature",
        "using bypass",
        "Caught a critical API error",
        "API call failed",
        "Quota exhausted",
        "quota_exceeded",
        "Waiting before re-evaluating",
    ]
    
    def __init__(self):
        super().__init__()
        self.setLevel(logging.INFO)
        # Track recent messages for deduplication: {msg_hash: (count, last_time, base_msg)}
        self._recent_msgs = {}
        self._dedup_window = 5.0  # seconds
    
    def _get_dedup_key(self, msg: str) -> str:
        """Extract a key for deduplication by stripping variable parts."""
        import re
        # Strip timestamps, numbers, specific IDs
        key = re.sub(r'\d+', 'N', msg)
        key = re.sub(r'toolu_[a-zA-Z0-9]+', 'TOOL_ID', key)
        key = re.sub(r'[a-f0-9]{8,}', 'HASH', key)
        return key[:100]  # Limit key length
    
    def _should_dedup(self, msg: str) -> bool:
        """Check if this message should be deduplicated."""
        for pattern in self.DEDUP_PATTERNS:
            if pattern in msg:
                return True
        return False
    ...
    def emit(self, record):
        try:
            ...
            # Skip LiteLLM messages
            if record.name.startswith("LiteLLM"):
                return
            
            # Check for deduplication
            if self._should_dedup(msg):
                dedup_key = self._get_dedup_key(msg)
                
                # Clean old entries
                expired_keys = [k for k, v in self._recent_msgs.items()
                               if current_time - v[1] > self._dedup_window]
                for k in expired_keys:
                    del self._recent_msgs[k]
                
                if dedup_key in self._recent_msgs:
                    # Increment count, don't print yet
                    count, _, base_msg = self._recent_msgs[dedup_key]
                    self._recent_msgs[dedup_key] = (count + 1, current_time, base_msg)
                    return
                else:
                    # First occurrence, track it
                    self._recent_msgs[dedup_key] = (1, current_time, msg)
            
            # Flush any pending deduplicated messages before printing new one
            self._flush_dedup_msgs()
            
            # Use Rich console for output with improved formatting
    ...
    def _flush_dedup_msgs(self):
        """Flush deduplicated messages with their counts."""
        import time
        current_time = time.time()
        
        for dedup_key, (count, last_time, base_msg) in list(self._recent_msgs.items()):
            if current_time - last_time > self._dedup_window / 2 and count > 1:
                # Print summary
                console.print(f"[bold yellow]⚠[/] [yellow]{base_msg}[/] [dim](×{count})[/]")
                del self._recent_msgs[dedup_key]
...
if WEB_UI_DIR.exists():
    app.mount("/dashboard", StaticFiles(directory=str(WEB_UI_DIR), html=True), name="dashboard")
else:
...
async def system_status(request: Request):
    """
    Returns general system status and uptime information.
    """
    uptime = time.time() - _start_time
    
    # Simple formatting of uptime
    ...
    return {
        ...
        "version": "1.0.0", # Could be pulled from package metadata
